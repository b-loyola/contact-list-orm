...
class Contact
  ...
  def save
    if id
      ...
      pg_contact = @@conn.exec_params('INSERT INTO contacts (name,email) VALUES ($1, $2) RETURNING *;', [name,email])
  ...
  def self.search(term)
    ...
    @@conn.exec_params('SELECT * FROM contacts WHERE name ILIKE $1 OR email ILIKE $1;', ['%'+term+'%']).inject([]) do |matches,contact|
      contact = Contact.new(contact["name"], contact["email"], contact["id"])
      matches << contact
    end
  end
